...
-(BOOL) needSwitchDB:(NSString*)uid {
    NSString *dbPrefix = @"QXQ";//[WKSDK shared].options.dbPrefix;
...
-(void) switchDB:(NSString*)uid {
    ...
    NSString *dbDir = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"QXQ"];
    NSString *dbPrefix = @"QXQ";
    NSFileManager *fileManager = [NSFileManager defaultManager];
   BOOL isExist = [fileManager fileExistsAtPath: dbDir];
    if(!isExist) {
        NSError *error;
        if (![fileManager createDirectoryAtPath:dbDir withIntermediateDirectories:YES attributes:nil error:&error]) {
            NSLog(@"creat Directory Failed:%@",[error localizedDescription]);
        }
    }
    NSString *dbPath = [NSString stringWithFormat:@"%@/%@%@.db",dbDir,dbPrefix,uid];
...
//        [db setKey:uid];
        ...
        [db executeUpdate:@"PRAGMA temp_store=MEMORY;"];
        [db executeUpdate:@"PRAGMA cache_size=-8000;"];

        [WFCCMessageDB sharedManager];
        [WFCCConversationDB sharedManager];
    }];
    // 合并数据库
