...
+ (instancetype)cardWithTarget:(NSString *)targetId type:(WFCCCardType)type from:(NSString *)fromUser {
    ...
    if (type == 0) {
        WFCCUserInfo *userInfo = [[WFCCUserDB sharedManager] getUserInfo:targetId];
        content.name = userInfo.name;
        content.displayName = (userInfo.alias.length > 0 ? userInfo.alias : userInfo.displayName);
...
- (NSString *)digest:(WFCCMessage *)message {
    NSString *card = ([WFCCIMService.main isChinese]?@"[名片]":@"[Business Card]");
