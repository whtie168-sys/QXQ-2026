...
- (NSString *)formatNotification:(WFCCMessage *)message {
    ...
    if ([[WFCCNetworkService sharedInstance].userId isEqualToString:self.operateUser]) {
        ...
        if (userInfo.alias.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.alias, (isChinese?@"解散了群聊":@" disbanded the group chat")];
        } else if(userInfo.groupAlias.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.groupAlias, (isChinese?@"解散了群聊":@" disbanded the group chat")];
        } else if (userInfo.displayName.length > 0) {
            formatMsg = [NSString stringWithFormat:@"%@%@", userInfo.displayName, (isChinese?@"解散了群聊":@" disbanded the group chat")];
        } else {
            if (isChinese) {
                formatMsg = [NSString stringWithFormat:@"用户<%@>解散了群聊", self.operateUser];
            }else {
                formatMsg = [NSString stringWithFormat:@"User <%@> disbanded the group chat", self.operateUser];
            }
        }
    }
