...
#import "GSXICTHAMessageVC.h"
#import <AVFoundation/AVFoundation.h>

#import "YUBWOIJWDAnnouncementCell.h"
#import "YUBWOIJWDAnnouncementMessageContent.h"


#import "GSXICTHAGroupAnnouncementVC.h"

#import "GSXICTHAGroupSetupVC.h"
#import "GSXICTHASingleSetupVC.h"

#import "RUJBVOGHUYMemberInfoVC.h"
#import "RUJBVOGHUYFriendInfoVC.h"
#import "GSXICTHAConversationSearchVC.h"
...
@interface GSXICTHAMessageVC ()<UITextFieldDelegate, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource, UICollectionViewDelegate, UINavigationControllerDelegate, ASDFGMessageCellDelegate, AVAudioPlayerDelegate, WDCARChatInputBarDelegate, UIGestureRecognizerDelegate, MWPhotoBrowserDelegate>  // QWERTMultiCallOngoingExpendedCellDelegate removed: voip
{
...
@implementation GSXICTHAMessageVC
...
- (void)topUserInfo {
    if (self.conversation.type == Single_Type) {
        [_iconView sd_setImageWithURL:URL(self.targetUser.portrait) placeholderImage: [QWERImage imageNamed:@"PersonalChat"]];
        if (self.targetUser.friendAlias.length) {
            _tzboeuNameLabel.text = self.targetUser.friendAlias;
        ...
        if ([self.conversation.target isEqualToString:@"customer_service"]) {
            ...
            if (self.targetUser.portrait.length <= 0) {
                _iconView.image = IMAGENAME(@"QXQ Messenger");
            }
            _tzboeuNameLabel.text = @"QXQ Messenger";
...
- (void)onRightBarBtn:(UIBarButtonItem *)sender {
    ...
    if (self.conversation.type == Group_Type) {
        GSXICTHAGroupSetupVC *vc = GSXICTHAGroupSetupVC.new;
        vc.conversation = self.conversation;
        vc.groupAnnouncement = _groupAnnouncement;
        [self.navigationController pushViewController:vc animated:YES];
    }else {
        GSXICTHASingleSetupVC *gvc = GSXICTHASingleSetupVC.new;
...
- (void)setupNavigationItem {
    ...
    // voip: videoVoice method removed (Chat86AVEngineKit/RADCOVideoVC)
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)sendChatroomWelcomeMessage {
    if(!self.silentJoinChatroom) {
        ...
        if (_isChinese) {
            ...
            tip.tip = UNString(@"Welcome %@ to the chat room", (userInfo.friendAlias.length > 0 ? userInfo.friendAlias : userInfo.displayName));
        }
    ...
    if(!self.silentJoinChatroom) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self->_isChinese) {
                ...
                tip.tip = UNString(@"%@ left the chat room", (userInfo.friendAlias.length > 0 ? userInfo.friendAlias : userInfo.displayName));
            }
...
- (void)onSearchBarBtn:(id)sender {
    if (self.multiSelecting) {
        ...
        GSXICTHAConversationSearchVC *mvc = [[GSXICTHAConversationSearchVC alloc] init];
    ...
    UIAlertAction *AllInOneAction = [UIAlertAction actionWithTitle:LLLLLL(@"MergeForward") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (self.conversation.type == Single_Type) {
            ...
            if (self->_isChinese) {
                ...
                compositeContent.title = [NSString stringWithFormat:@"%@ and %@'s chat history", title, myself.displayName];
            }
        } else if (self.conversation.type == Group_Type) {
...
- (void)initializedSubViews {
    ...
    // [self registerCell:[ASDFGConferenceInviteCell class] forContent:[WFCCConferenceInviteMessageContent class]]; // removed: channel
    ...
    // 自定义群公告消息 1130
    [self registerCell:[YUBWOIJWDAnnouncementCell class] forContent:[YUBWOIJWDAnnouncementMessageContent class]];
...
- (void)onMessageReaded:(NSNotification *)notification {
    ...
    // voip: onCallStateChanged removed] systemVersion] rangeOfString:@"10."].location == 0) {
            [self.chatInputBar resetInputBarStatue];
        }
    }
}
...
-(void)startPlay:(QWERTMessageModel *)model {
    ...
    if ([model.message.content isKindOfClass:[WFCCSoundMessageContent class]]) {
        ...
        if (!url) {
            [self.view makeToast:_isChinese?@"无法播放":@"Cannot be played"];
            return;
        }
...
- (void)didTapMessageCell:(ASDFGMessageCellBase *)cell withModel:(QWERTMessageModel *)model {
    if ([model.message.content isKindOfClass:[YUBWOIJWDAnnouncementMessageContent class]]) { // 1130新增。公告点击事件
        ...
        ann.text = [(YUBWOIJWDAnnouncementMessageContent *)model.message.content text];
        ann.timestamp = model.message.serverTime;
        
        GSXICTHAGroupAnnouncementVC *vc = [[GSXICTHAGroupAnnouncementVC alloc] init];
        vc.isCanPost = NO;
        vc.announcement = ann;
        ...
        if ([channelInfo.owner isEqualToString:[WFCCNetworkService sharedInstance].userId]) {
            ...
            UIAlertAction *okAction = [UIAlertAction actionWithTitle:(_isChinese?@"与订阅者私聊":@"Chat privately with subscribers") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                if (model.message.direction == MessageDirection_Receive) {
                    WFCCChannelInfo *channelInfo = [[WFCCIMService sharedWFCIMService] getChannelInfo:self.conversation.target refresh:NO];
                    if ([channelInfo.owner isEqualToString:[WFCCNetworkService sharedInstance].userId]) {
                        GSXICTHAMessageVC *mvc = [[GSXICTHAMessageVC alloc] init];
...
- (void)didSelectUrl:(ASDFGMessageCellBase *)cell withModel:(QWERTMessageModel *)model withUrl:(NSString *)urlString {
    if ([model.message.content.class isEqual:NSClassFromString(@"YUBWOIJWDAnnouncementMessageContent")]) { // 群公告点击链接
...
- (void)showQuote:(WFCCQuoteInfo *)quoteInfo ofMessage:(WFCCMessage *)msg {
    if ([msg.content isKindOfClass:[WFCCTextMessageContent class]]) {
    ...
    } else if ([msg.content isKindOfClass:[YUBWOIJWDAnnouncementMessageContent class]]) { // 1130新增 引用群公告引用
        //        WS(weakself)
        ...
        ann.text = [(YUBWOIJWDAnnouncementMessageContent *)msg.content text];
...
- (void)didTaptzboeuQuoteLabel:(ASDFGMessageCellBase *)cell withModel:(QWERTMessageModel *)model {
    if ([model.message.content isKindOfClass:[WFCCTextMessageContent class]]) {
...
- (void)didSelectSticker:(NSString *)stickerPath {
    ...
    // voip: didTouchVideoBtn method removed
...
- (void)displayMenu:(ASDFGMessageCellBase *)baseCell {
    ...
    if ([msg.content isKindOfClass:[WFCCTextMessageContent class]]/* || [msg.content isKindOfClass:YUBWOIJWDAnnouncementMessageContent.class]*/) {
    ...
    if (_conversation.type == Group_Type) {
        if ([msg.content isKindOfClass:[WFCCTextMessageContent class]] ||
            [msg.content isKindOfClass:[WFCCFileMessageContent class]] ||
            [msg.content isKindOfClass:[YUBWOIJWDAnnouncementMessageContent class]]) {
...
//    }
//}
    // voip: UITableView delegate/datasource methods removed
...
- (void)dealloc {
    ...
    for (NSInteger i = 0; i<self.modelList.count; i++) {
        ...
        if (model.message.direction == MessageDirection_Send) { // 如果是发送方 则需要判断该条消息是否已读
            if (((ASDFGMessageCell *)cell).tzboeuUnreadButton.selected == NO || isRead == NO) { // 该条消息没有人已读
                continue;
            }
        }else {
...
- (void)clickTopMessage {
    ...
    [popupView setClickBlock:^(NSInteger type) {
        if (type == 2000) {
            [weakself enterGroupAnnouncement:YES];
        }else {
