...
@interface NFCNUOEYForwardVC ()<UITableViewDataSource, UISearchControllerDelegate, UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating>
{
    BOOL _isChinese;
}
...
@implementation NFCNUOEYForwardVC
...
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"NRUJBVOGHUYContactsTVCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"NRUJBVOGHUYContactsTVCell"];
...
- (void)sortAndRefreshWithList:(NSArray *)friendList {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        self.resultDic = [NFCNUOEYForwardVC sortedArrayWithPinYinDic:friendList];
    ...
    if (self.searchController.active || self.selectContact) {
        ...
        if (indexPath.section == 0) {
            if (indexPath.row == 0) {
                ...
                HNWOUIDContactTVCell *contactCell = [self dequeueOrAllocChannelCell:tableView];
                contactCell.separatorInset = UIEdgeInsetsMake(0, 80.0, 0, 0);
...
//            cell = contactCell;
            ...
            NRUJBVOGHUYContactsTVCell *cell = [tableView dequeueReusableCellWithIdentifier:@"NRUJBVOGHUYContactsTVCell" forIndexPath:indexPath];
            WFCCUserInfo *userInfo = dataSource[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.searchController.active || self.selectContact) {
        ...
        if (indexPath.section == 0) {
            //选择朋友
            if (indexPath.row == 0) {
                SelectRUJBVOGHUYContactVC *vc = SelectRUJBVOGHUYContactVC.new;
                vc.message = self.message;
                vc.messages= self.messages;
                vc.hidesBottomBarWhenPushed = YES;
                [self.navigationController pushViewController:vc animated:YES];
            } else if(indexPath.row == 1) { // 我的群组
                SelectRUJBVOGHUYGroupVC *groupVC = [[SelectRUJBVOGHUYGroupVC alloc] init];;
                groupVC.message = self.message;
                groupVC.messages= self.messages;
                groupVC.hidesBottomBarWhenPushed = YES;
                [self.navigationController pushViewController:groupVC animated:YES];
            } else if(indexPath.row == 2) {
                SelectRUJBVOGHUYTagVC *tagVC = [[SelectRUJBVOGHUYTagVC alloc] init];
...
+ (NSMutableDictionary *)sortedArrayWithPinYinDic:(NSArray *)userList {
    ...
    [infoDic enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        ...
        [_tempOtherArr sortUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
            ...
            NSString *user1Pinyin = [NFCNUOEYForwardVC hanZiToPinYinWithString:user1.finalName];
            NSString *user2Pinyin = [NFCNUOEYForwardVC hanZiToPinYinWithString:user2.finalName];
