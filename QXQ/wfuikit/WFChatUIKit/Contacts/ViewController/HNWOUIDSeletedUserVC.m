...
@interface HNWOUIDSeletedUserVC () <UITableViewDataSource, UITableViewDelegate,
UICollectionViewDataSource, UICollectionViewDelegate,
UISearchBarDelegate, HNWOUIDSelectedUserTVCellDelegate>
...
@property(nonatomic, strong)NSMutableArray<LUDHIOWIVOrganization *> *organizations;
@property(nonatomic, strong)NSMutableArray<LUDHIOWIVEmployee *> *employees;
...
@implementation HNWOUIDSeletedUserVC
...
- (void)viewDidLoad {
    ...
    if(!self.disabledUserNotSelected) {
        for (NSString *defaultUserId in self.disableUserIds) {
            HNWOUIDSelectModel *defaultUser = [[HNWOUIDSelectModel alloc] init];
            defaultUser.selectedStatus = Disable_Checked;
            WFCCUserInfo *userInfo = [[WFCCUserDB sharedManager] getUserInfo:defaultUserId inGroup:self.groupId];//[[WFCCIMService sharedWFCIMService] getUserInfo:defaultUserId inGroup:self.groupId refresh:NO];
...
- (void)updateNavi {
    if(self.organizationIds.count) {
        ...
        UIBarButtonItem *close = [[UIBarButtonItem alloc] initWithImage:[AIOIUEHImage imageNamed:@"close"] style:UIBarButtonItemStylePlain target:self action:@selector(cancel)];
...
#pragma mark - HNWOUIDSelectedUserTVCellDelegate
- (void)didTapNextLevel:(HNWOUIDSelectModel *)model {
    if(!self.organizationIds.count) {
    ...
    LUDHIOWIVOrganizationEx *ex = [[LUDHIOWIVOrganizationEx alloc] init];
...
- (BOOL)searchBarShouldBeginEditing:(UISearchBar *)searchBar {
    HNWOUIDSeletedUserSearchResultVC *resultVC = [[HNWOUIDSeletedUserSearchResultVC alloc] init];
    ...
    resultVC.selectedUserBlock = ^(HNWOUIDSelectModel * _Nonnull user) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    HNWOUIDSelectedUserTVCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    cell.delegate = self;
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    HNWOUIDSelectedUserCVCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"selectedUserC" forIndexPath:indexPath];
...
- (void)loadData {
    ...
    if(self.organizationIds.count) {
        ...
        [[LUDHIOWIVOrganizationCache sharedCache] getOrganizationEx:orgId refresh:NO success:^(NSInteger organizationId, LUDHIOWIVOrganizationEx * _Nonnull ex) {
            [hud hideAnimated:NO];
            if(organizationId == [ws.organizationIds.lastObject integerValue]) {
                LUDHIOWIVOrganizationEx *p = [ws.paths lastObject];
        ...
        if (self.inputData) {
            ...
            userDataSource = [[WFCCUserDB sharedManager] getUserInfos:self.candidateUsers];//[[WFCCIMService sharedWFCIMService] getUserInfos:self.candidateUsers inGroup:nil];
        } else {
//            NSArray *userIdList = [[WFCCIMService sharedWFCIMService] getMyFriendList:YES];
            userDataSource = [[WFCCUserDB sharedManager] getAllFriendInfos]; //[[WFCCIMService sharedWFCIMService] getUserInfos:userIdList inGroup:nil];
        }
        
        for (WFCCUserInfo *userInfo in userDataSource) {
            __block HNWOUIDSelectModel *info = [[HNWOUIDSelectModel alloc] init];
            info.userInfo = userInfo;
            if ([self.disableUserIds containsObject:info.userInfo.userId]) {
                info.selectedStatus = Disable_Checked;
            }
            [self.selectedUsers enumerateObjectsUsingBlock:^(HNWOUIDSelectModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        NSMutableArray<NSNumber *> *ids = [[[LUDHIOWIVOrganizationCache sharedCache] rootOrganizationIds] mutableCopy];
        [ids addObjectsFromArray:[LUDHIOWIVOrganizationCache sharedCache].bottomOrganizationIds];
        if(ids.count) {
            ...
            [ids enumerateObjectsUsingBlock:^(NSNumber * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                LUDHIOWIVOrganization *org = [[LUDHIOWIVOrganizationCache sharedCache] getOrganization:[obj integerValue] refresh:NO];
                if(org) {
                    [orgs addObject:org];
                }
            }];
...
//    [[AIOIUEHConfigManager globalManager] setupNavBar];
    ...
    for (HNWOUIDSelectModel *user in self.selectedUsers) {
    ...
    if(orgIds.count) {
        ...
        [[AIOIUEHConfigManager globalManager].orgServiceProvider getBatchOrgEmployees:orgIds success:^(NSArray<NSString *> * _Nonnull employeeIds) {
            [hud hideAnimated:NO];
...
- (void)mergeOrgAndEmps {
    ...
    LUDHIOWIVOrganizationEx *ex = [self.paths lastObject];
    if(ex.employees.count) {
        NSMutableArray *emps = [[NSMutableArray alloc] init];
        [ex.employees enumerateObjectsUsingBlock:^(LUDHIOWIVEmployee * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            HNWOUIDSelectModel *model = [[HNWOUIDSelectModel alloc] init];
            model.employee = obj;
            model.selectedStatus = Unchecked;
            [self.selectedUsers enumerateObjectsUsingBlock:^(HNWOUIDSelectModel * _Nonnull obj1, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if(ex.subOrganizations.count) {
        NSMutableArray *orgs = [[NSMutableArray alloc] init];
        [ex.subOrganizations enumerateObjectsUsingBlock:^(LUDHIOWIVOrganization * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            HNWOUIDSelectModel *model = [[HNWOUIDSelectModel alloc] init];
...
- (BOOL)toggelSeletedUser:(HNWOUIDSelectModel *)user {
...
- (void)reloadCellForUser:(HNWOUIDSelectModel *)user {
    for (NSString *key in self.sectionKeys) {
        NSArray *users = self.sectionDictionary[key];
        for (HNWOUIDSelectModel *u in users) {
            if ([u isEqual:user]) {
...
- (UICollectionView *)selectedUserCollectionView {
    if (!_selectedUserCollectionView) {
        ...
        [_selectedUserCollectionView registerClass:[HNWOUIDSelectedUserCVCell class] forCellWithReuseIdentifier:@"selectedUserC"];
...
//        _tableView.sectionIndexColor = [UIColor colorWithHexString:@"0x4e4e4e"];
        ...
        [_tableView registerClass:[HNWOUIDSelectedUserTVCell class] forCellReuseIdentifier:@"cell"];
...
- (UIView *)topView {
    if (!_topView) {
        _topView = [UIView new];
        if (self.type == Horizontal) {
            _topView.backgroundColor = [AIOIUEHConfigManager globalManager].naviBackgroudColor;
