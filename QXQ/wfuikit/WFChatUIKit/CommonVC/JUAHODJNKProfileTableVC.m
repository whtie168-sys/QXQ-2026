...
#import "AIOIUEHUserMessageListVC.h"
#import "AIOIUEHImage.h"
#import "JUAHODJNKProfileMoreTableVC.h"
...
@interface JUAHODJNKProfileTableVC () <UITableViewDelegate, UITableViewDataSource, MWPhotoBrowserDelegate>
@property (strong, nonatomic)UIImageView *trewqPortraitView;
@property (strong, nonatomic)UILabel *aliasLabel;
@property (strong, nonatomic)UILabel *displaytzboeuNameLabel;
@property (strong, nonatomic)UILabel *usertzboeuNameLabel;
...
@implementation JUAHODJNKProfileTableVC
...
- (void)loadOrganizationData:(BOOL)refresh {
    LUDHIOWIVEmployeeEx *employeeEx = [[LUDHIOWIVOrganizationCache sharedCache] getEmployeeEx:self.userId refresh:refresh];
    if(employeeEx.relationships.count) {
        NSMutableArray<NSNumber *> *arr = [[NSMutableArray alloc] init];
        [employeeEx.relationships enumerateObjectsUsingBlock:^(LUDHIOWIVOrgRelationship * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if(obj.bottom) {
...
- (void)onRightBtn:(id)sender {
    ...
    if ([[WFCCIMService sharedWFCIMService] isMyFriend:self.userId]) {
        ...
        UIAlertAction *addFriendAction = [UIAlertAction actionWithTitle:WFCString(@"AddFriend") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            JUAHODJNKVerifyRequestVC *vc = [[JUAHODJNKVerifyRequestVC alloc] init];
            vc.userId = ws.userId;
...
- (void)loadData {
    ...
    [self.trewqPortraitView sd_setImageWithURL:[NSURL URLWithString:[self.userInfo.portrait stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholderImage: [AIOIUEHImage imageNamed:@"PersonalChat"] options:SDWebImageScaleDownLargeImages
    ...
    if (alias.length) {
        ...
        self.displaytzboeuNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(94, 32, width - 94 - 8, 21)];
        self.displaytzboeuNameLabel.font = [UIFont pingFangSCWithWeight:FontWeightStyleRegular size:14];
        self.displaytzboeuNameLabel.text = self.userInfo.displayName;
        
        self.usertzboeuNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(94, 60, width - 94 - 8, 11)];
        self.usertzboeuNameLabel.text = [NSString stringWithFormat:@"ID: %@", self.userInfo.name];
        self.usertzboeuNameLabel.font = [UIFont pingFangSCWithWeight:FontWeightStyleRegular size:12];
        self.usertzboeuNameLabel.textColor = [UIColor grayColor];
    } else {
        self.aliasLabel = [[UILabel alloc] initWithFrame:CGRectZero];

        self.displaytzboeuNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(94, 23, width - 94 - 8, 21)];
        self.displaytzboeuNameLabel.text = self.userInfo.displayName;
        self.displaytzboeuNameLabel.font = [UIFont pingFangSCWithWeight:FontWeightStyleMedium size:20];
        
        self.usertzboeuNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(94, 50, width - 94 - 8, 21)];
        self.usertzboeuNameLabel.text = [NSString stringWithFormat:@"ID: %@", self.userInfo.name];
        self.usertzboeuNameLabel.font = [UIFont pingFangSCWithWeight:FontWeightStyleRegular size:12];
        self.usertzboeuNameLabel.textColor = [UIColor grayColor];
    }
    if (self.fromConversation.type == Group_Type) {
        if (self.isManager) { // 是管理员 可以看到ID
            self.usertzboeuNameLabel.hidden = NO;
        }else {
            if ([[WFCCIMService sharedWFCIMService] isMyFriend:self.userId]) { // 是好友关系
                ...
                if ([[WFCCNetworkService sharedInstance].userId isEqualToString:self.userId]) {
                    ...
                    self.usertzboeuNameLabel.hidden = YES;
                }
            }
        }
    }
    ...
    if ([[WFCCIMService sharedWFCIMService] isFavUser:self.userId]) {
        self.starLabel = [[UILabel alloc] initWithFrame:CGRectMake(width - 16 - 20, self.displaytzboeuNameLabel.frame.origin.y, 20, 20)];
        self.starLabel.text = @"☆";
    ...
    [self.headerCell.contentView addSubview:self.displaytzboeuNameLabel];
    [self.headerCell.contentView addSubview:self.usertzboeuNameLabel];
    ...
    if (self.userInfo.type == 1) {
        ...
        if ([[WFCCIMService sharedWFCIMService] isMyFriend:self.userId]) {
            ...
            [btn setTitleColor:[AIOIUEHConfigManager globalManager].textColor forState:UIControlStateNormal];
        ...
        if (self.userInfo.type == 0) {
            if(NSClassFromString(@"SDTimeLineTableViewController")) {
                ...
                [momentButton setTitleColor:[AIOIUEHConfigManager globalManager].textColor forState:UIControlStateNormal];
        ...
        if ([[WFCCIMService sharedWFCIMService] isMyFriend:self.userId]) {
            ...
            [btn setImage:[AIOIUEHImage imageNamed:@"video"] forState:UIControlStateNormal];
...
- (void)setupSendMessageCell:(CGFloat)width {
    ...
    UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, width, 50)];
    [btn setImage:[AIOIUEHImage imageNamed:@"message"] forState:UIControlStateNormal];
    btn.imageEdgeInsets = UIEdgeInsetsMake(0, 0, 0, 10);
...
- (void)onSendMessageBtn:(id)sender {
    ...
    for (UIViewController *vc in self.navigationController.viewControllers) {
        if ([vc isKindOfClass:[AIOIUEHMessageListVC class]]) {
            AIOIUEHMessageListVC *old = (AIOIUEHMessageListVC*)vc;
    ...
    if (indexPath.section == 0) {
       return self.headerCells[indexPath.row];
        ...
        if(index < self.organizationIds.count) {
            ...
            LUDHIOWIVOrganization *org = [[LUDHIOWIVOrganizationCache sharedCache] getOrganization:orgId refresh:NO];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([tableView cellForRowAtIndexPath:indexPath] == self.userMessagesCell) {
        AIOIUEHUserMessageListVC *vc = [[AIOIUEHUserMessageListVC alloc] init];
        vc.userId = self.userId;
        vc.conversation = self.fromConversation;
        [self.navigationController pushViewController:vc animated:YES];
    } else if([tableView cellForRowAtIndexPath:indexPath] == self.moreCell) {
        JUAHODJNKProfileMoreTableVC *moreVC = [[JUAHODJNKProfileMoreTableVC alloc] init];
        ...
        if(index < self.organizationIds.count) {
            NSInteger orgId = [self.organizationIds[index] integerValue];
            NSArray<LUDHIOWIVOrgRelationship *> *rs = [[LUDHIOWIVOrganizationCache sharedCache] getRelationship:self.userId refresh:YES];
