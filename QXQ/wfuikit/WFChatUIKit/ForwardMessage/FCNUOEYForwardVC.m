...
@interface FCNUOEYForwardVC () <UITableViewDataSource, UISearchControllerDelegate, UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating>
...
@implementation FCNUOEYForwardVC
...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:WFCString(@"Done") style:UIBarButtonItemStyleDone target:self action:@selector(onLeftBarBtn:)];
    ...
    if (@available(iOS 13, *)) {
        ...
        self.searchController.searchBar.searchTextField.backgroundColor = [UIColor colorWithHexString:@"#F6F6F6"];//[AIOIUEHConfigManager globalManager].naviBackgroudColor;
...
- (void)altertSend:(WFCCConversation *)conversation {
    FCNUOEYShareMessageView *shareView = [FCNUOEYShareMessageView createViewFromNib];
...
#define REUSENEWCONVIDENTIFY @"resueNewConvCell"
    if (self.searchController.active) {
        ...
        if (indexPath.section == 0) {
            ...
            FCNUOEYForwardMessageCell *cell = [tableView dequeueReusableCellWithIdentifier:REUSECONVIDENTIFY];
            if(!cell) {
                cell = [[FCNUOEYForwardMessageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:REUSECONVIDENTIFY];
            }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (self.searchController.isActive) {
        if (self.searchGroupList.count + self.searchFriendList.count > 0) {
            ...
            label.backgroundColor = [AIOIUEHConfigManager globalManager].backgroudColor;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.searchController.isActive) {
        ...
        if (indexPath.section == 0) { //new conversation
            // HNWOUIDContactListVC *pvc = [[HNWOUIDContactListVC alloc] init]; // removed
            HNWOUIDSeletedUserVC *pvc = [[HNWOUIDSeletedUserVC alloc] init];
            // pvc.selectContact = YES; // removed: property not on SeletedUserVC
            // pvc.multiSelect = NO; // removed: property not on SeletedUserVC
            // pvc.isPushed = YES; // removed: property not on SeletedUserVC
            __weak typeof(self)ws = self;
