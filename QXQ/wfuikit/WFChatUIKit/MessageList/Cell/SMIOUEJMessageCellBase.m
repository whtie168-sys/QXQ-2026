...
@implementation SMIOUEJMessageCellBase
...
- (void)setModel:(AIOIUEHMessageModel *)model {
  ...
  _model = model;
    ...
    if (model.lastReadMessage) {
        if (!self.lastReadContainerView) {
            ...
            CGSize size = [AIOIUEHUtilities getTextDrawingSize:label.text font:label.font constrainedSize:CGSizeMake(screenWidth-16, 8000)];
            size.width += 16;
  ...
  if (model.showTimeLabel) {
    ...
    _timeLabel.text = [AIOIUEHUtilities formatTimeDetailLabel:model.message.serverTime];

    
    CGSize size = [AIOIUEHUtilities getTextDrawingSize:_timeLabel.text font:_timeLabel.font constrainedSize:CGSizeMake(screenWidth, 8000)];
