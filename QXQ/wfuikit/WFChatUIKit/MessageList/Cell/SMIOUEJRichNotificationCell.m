...
@interface SMIOUEJRichNotificationCell ()
...
@implementation SMIOUEJRichNotificationCell
+ (CGSize)sizeForCell:(AIOIUEHMessageModel *)msgModel withViewWidth:(CGFloat)width {
    ...
    CGSize titleSize = [AIOIUEHUtilities getTextDrawingSize:content.title font:[UIFont systemFontOfSize:TITLE_FONT_SIZE] constrainedSize:CGSizeMake(containerWidth-CELL_PADDING-CELL_PADDING, 50)];
    
    
    CGSize descSize = [AIOIUEHUtilities getTextDrawingSize:content.desc font:[UIFont systemFontOfSize:FONT_SIZE] constrainedSize:CGSizeMake(containerWidth-CELL_PADDING-CELL_PADDING, 50)];
    
    CGFloat itemsHeight = 0;
    for (NSDictionary<NSString*, NSString*> *data in content.datas) {
        NSString *value = data[@"value"];
        CGSize itemSize = [AIOIUEHUtilities getTextDrawingSize:value font:[UIFont systemFontOfSize:FONT_SIZE] constrainedSize:CGSizeMake(containerWidth-CELL_PADDING-CELL_PADDING-VALUE_BOARD_PADDING_LEFT, 50)];
    ...
    CGSize remarkSize = [AIOIUEHUtilities getTextDrawingSize:content.remark font:[UIFont systemFontOfSize:FONT_SIZE] constrainedSize:CGSizeMake(containerWidth-CELL_PADDING-CELL_PADDING, 50)];
...
- (void)setModel:(AIOIUEHMessageModel *)model {
    ...
    if(content.exName.length) {
        ...
        [self.extrewqPortraitView sd_setImageWithURL:[NSURL URLWithString:content.exPortrait] placeholderImage:[AIOIUEHImage imageNamed:@"default_app_icon"] options:SDWebImageScaleDownLargeImages
...
- (CGFloat)addValueLabel:(CGFloat)offset text:(NSString *)text color:(NSString *)colorStr {
    ...
    CGFloat containerWidth = [UIScreen mainScreen].bounds.size.width - CELL_MARGIN - CELL_MARGIN;
    CGSize itemSize = [AIOIUEHUtilities getTextDrawingSize:text font:[UIFont systemFontOfSize:FONT_SIZE] constrainedSize:CGSizeMake(containerWidth-CELL_PADDING-CELL_PADDING-VALUE_BOARD_PADDING_LEFT, 50)];
...
- (CGFloat)setLabel:(UILabel *)label widht:(CGFloat)width text:(NSString *)text offset:(CGFloat)offset fontSize:(int)fontSize {
    CGSize titleSize = [AIOIUEHUtilities getTextDrawingSize:text font:[UIFont systemFontOfSize:fontSize] constrainedSize:CGSizeMake(width, 50)];
    label.text = text;
...
- (UIView *)exView {
    if(!_exView) {
        ...
        _exFWView.image = [AIOIUEHImage imageNamed:@"forward_normal"];
