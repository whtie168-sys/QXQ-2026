...
- (instancetype)initWithDelegate:(id<WDCARPluginBoardViewDelegate>)delegate withVoip:(BOOL)withWoip withPtt:(BOOL)withPtt {
    ...
    if (self) {
        ...
        self.hasPtt = withPtt;
        self.backgroundColor = [AIOIUEHConfigManager globalManager].backgroudColor;
...
- (NSMutableArray *)pluginItems {
    if (!_pluginItems) {
        ...
        _pluginItems = [@[
            [[PluginItem alloc] initWithTitle:(isChinese?@"相册":@"Album") image:[AIOIUEHImage imageNamed:@"chat_input_plugin_album1"] tag:1],
            [[PluginItem alloc] initWithTitle:(isChinese?@"拍摄":@"Camera") image:[AIOIUEHImage imageNamed:@"chat_input_plugin_camera1"] tag:2],
            [[PluginItem alloc] initWithTitle:(isChinese?@"位置":@"Location") image:[AIOIUEHImage imageNamed:@"chat_input_plugin_location1"] tag:3],
            [[PluginItem alloc] initWithTitle:(isChinese?@"文件":@"Files") image:[AIOIUEHImage imageNamed:@"chat_input_plugin_file1"] tag:5],
            [[PluginItem alloc] initWithTitle:(isChinese?@"名片":@"Card") image:[AIOIUEHImage imageNamed:@"chat_input_plugin_card1"] tag:6]
                          ] mutableCopy];
...
//        }
        ...
        if (self.hasVoip) {
            [_pluginItems insertObject:[[PluginItem alloc] initWithTitle:(isChinese?@"视频通话":@"Video call") image:[AIOIUEHImage imageNamed:@"chat_input_plugin_video_call1"] tag:4] atIndex:2];
        }
#endif
#ifdef WFC_PTT
        if(self.hasPtt) {
            [_pluginItems addObject:[[PluginItem alloc] initWithTitle:(isChinese?@"对讲":@"Talk") image:[AIOIUEHImage imageNamed:@"chat_input_plugin_intercom"] tag:7]];
        }
#endif
    }
    return _pluginItems;
