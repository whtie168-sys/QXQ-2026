...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = (_isChinese?@"位置":@"Location");
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:(_isChinese?@"取消":@"Cancel") style:UIBarButtonItemStylePlain target:self action:@selector(dismiss:)];
    ...
    if (_locationManager) {
        ...
        UIImage *image = [AIOIUEHImage imageNamed:@"PinGreen"];
        ...
        if ([CLLocationManager locationServicesEnabled]) {
            ...
            if (status == kCLAuthorizationStatusRestricted || status == kCLAuthorizationStatusDenied) {
                [self.view makeToast:(_isChinese?@"请在设置-隐私里允许程序使用地理位置服务":@"Please allow applications to use location services in Settings - Privacy")
                            duration:2
            ...
            [self.view makeToast:(_isChinese?@"请打开地理位置服务":@"Please turn on location services")
                        duration:2
                        position:CSToastPositionCenter];
        }
...
- (void)setUpRightNavButton{
    UIBarButtonItem *item = [[UIBarButtonItem alloc] initWithTitle:(_isChinese?@"选定":@"Select") style:UIBarButtonItemStyleDone target:self action:@selector(onSend:)];
...
- (void)reverseGeoLocation:(CLLocationCoordinate2D)locationCoordinate2D{
    ...
    [self.geoCoder reverseGeocodeLocation:location
         ...
         if (!error) {
             ...
             ws.title = (self->_isChinese?@"位置":@"Location");
         }
     }];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell.selected) {
        cell.imageView.image = [AIOIUEHImage imageNamed:@"multi_selected"];
    } else {
