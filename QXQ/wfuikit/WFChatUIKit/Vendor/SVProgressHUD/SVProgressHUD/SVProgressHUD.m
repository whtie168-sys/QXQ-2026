...
+ (void)showLogoWithStatus:(NSString*)status {
    [self showImage:[self sharedView].logoImage status:status];
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        _logoImage = [UIImage imageWithContentsOfFile:[imageBundle pathForResource:@"icon-28" ofType:@"png"]];
...
- (void)setLogoImage:(UIImage *)image {
    if (!_isInitializing) _logoImage = image;
}
