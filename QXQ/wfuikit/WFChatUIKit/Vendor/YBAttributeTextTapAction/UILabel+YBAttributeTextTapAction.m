...
- (BOOL)yb_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            NSMutableArray<NSNumber *> *lineCandidateIndexes = [NSMutableArray array];
            NSInteger nearestIndex = NSNotFound;
            CGFloat nearestDistance = CGFLOAT_MAX;
            CGFloat hitSlop = self.enlargeTapArea ? 18.f : 8.f;
            for (int j = 0; j < link_count; j++) {
                YBAttributeModel *model = self.attributeStrings[j];
                NSRange link_range = model.range;
                NSRange lineNSRange = NSMakeRange(lineRange.location, lineRange.length);
                NSRange intersection = NSIntersectionRange(link_range, lineNSRange);
                if (intersection.length == 0) {
                    continue;
                }
                [lineCandidateIndexes addObject:@(j)];
                
                CGFloat startOffset = CTLineGetOffsetForStringIndex(line, intersection.location, NULL);
                CFIndex endIndex = intersection.location + intersection.length;
                CGFloat endOffset = CTLineGetOffsetForStringIndex(line, endIndex, NULL);
                if (intersection.length > 0) {
                    CGFloat lastCharOffset = CTLineGetOffsetForStringIndex(line, endIndex - 1, NULL);
                    NSAttributedString *subString = [self.attributedText attributedSubstringFromRange:intersection];
                    CGFloat trailingWidth = [self yb_textSizeWithAttributedString:subString width:CGFLOAT_MAX numberOfLines:1].width;
                    endOffset = MAX(endOffset, lastCharOffset + trailingWidth);
                }
                if (endOffset < startOffset) {
                    CGFloat temp = startOffset;
                    startOffset = endOffset;
                    endOffset = temp;
                }
                CGFloat minX = startOffset - hitSlop;
                CGFloat maxX = endOffset + hitSlop;
                CGFloat distance = 0.f;
                if (relativePoint.x < minX) {
                    distance = minX - relativePoint.x;
                } else if (relativePoint.x > maxX) {
                    distance = relativePoint.x - maxX;
                }
            ...
            if (lineCandidateIndexes.count == 1) {
                NSInteger onlyIndex = lineCandidateIndexes.firstObject.integerValue;
                YBAttributeModel *model = self.attributeStrings[onlyIndex];
                if (resultBlock) {
                    resultBlock(model.str, model.range, onlyIndex);
                }
                CFRelease(frame);
                CFRelease(framesetter);
                CGPathRelease(Path);
            ...
            if (nearestIndex != NSNotFound && nearestDistance <= 26.f) {
                YBAttributeModel *model = self.attributeStrings[nearestIndex];
                if (resultBlock) {
                    resultBlock(model.str, model.range, nearestIndex);
                }
                CFRelease(frame);
                CFRelease(framesetter);
                CGPathRelease(Path);
...
- (CGRect)yb_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    ...
    CGFloat height = ceil(ascent + descent + MAX(leading, 0.0f));
    if (height <= 0.0f) {
        height = ceil(self.font.lineHeight);
    }
