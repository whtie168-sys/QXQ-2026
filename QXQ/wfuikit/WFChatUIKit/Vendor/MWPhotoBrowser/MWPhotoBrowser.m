...
- (void)viewDidLoad {
    ...
    if (self.displayNavArrows) {
        ...
        UIImage *qoynruNextButtonImage = [UIImage imageForResourcePath:[NSString stringWithFormat:arrowPathFormat, @"Right"] ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]];
        ...
        _qoynruNextButton = [[UIBarButtonItem alloc] initWithImage:qoynruNextButtonImage style:UIBarButtonItemStylePlain target:self action:@selector(gotoNextPage)];
    }
...
- (void)performLayout {
    ...
    if (_enableGrid) {
        ...
        //隐藏左下角
        [items addObject:[[UIBarButtonItem alloc] initWithImage:[UIImage imageForResourcePath:@"MWPhotoBrowser.bundle/UIBarButtonItemGrid" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] style:UIBarButtonItemStylePlain target:self action:@selector(saveAct)]];
//        [items addObject:[[UIBarButtonItem alloc] initWithImage:[UIImage imageForResourcePath:@"MWPhotoBrowser.bundle/UIBarButtonItemBaocun" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] style:UIBarButtonItemStylePlain target:self action:@selector(saveAct)]];
    ...
    // Middle - Nav
    if (_previousButton && _qoynruNextButton && numberOfPhotos > 1) {
        hasItems = YES;
        [items addObject:flexSpace];
        [items addObject:_previousButton];
        [items addObject:flexSpace];
        [items addObject:_qoynruNextButton];
        [items addObject:flexSpace];
    } else {
        [items addObject:flexSpace];
    }
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    _viewIsActive = YES;
    if (self.currentVideoPlayerViewController && self.presentedViewController != self.currentVideoPlayerViewController) {
        [self clearCurrentVideo];
    }
...
- (void)layoutVisiblePages {
    ...
    for (MWZoomingScrollView *page in _visiblePages) {
        ...
        if (page.videoLoadingLabel) {
            page.videoLoadingLabel.frame = [self frameForVideoLoadingLabel:page.videoLoadingLabel belowPlayButton:page.playButton atIndex:index];
        }
...
- (void)tilePages {
    ...
    for (MWZoomingScrollView *page in _visiblePages) {
        ...
        if (pageIndex < (NSUInteger)iFirstIndex || pageIndex > (NSUInteger)iLastIndex) {
            ...
            [page.videoLoadingLabel removeFromSuperview];
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            ...
            if (page.displayingVideo) {
                ...
                UILabel *videoLoadingLabel = [[UILabel alloc] initWithFrame:CGRectZero];
                videoLoadingLabel.text = @"视频加载中...";
                videoLoadingLabel.textColor = [UIColor colorWithWhite:1 alpha:0.9];
                videoLoadingLabel.font = [UIFont systemFontOfSize:14 weight:UIFontWeightMedium];
                videoLoadingLabel.textAlignment = NSTextAlignmentCenter;
                videoLoadingLabel.backgroundColor = [UIColor clearColor];
                videoLoadingLabel.hidden = YES;
                [videoLoadingLabel sizeToFit];
                videoLoadingLabel.frame = [self frameForVideoLoadingLabel:videoLoadingLabel belowPlayButton:playButton atIndex:index];
                [_pagingScrollView addSubview:videoLoadingLabel];
                page.videoLoadingLabel = videoLoadingLabel;
            }
...
- (CGRect)frameForVideoLoadingLabel:(UILabel *)label belowPlayButton:(UIButton *)playButton atIndex:(NSUInteger)index {
    CGRect pageFrame = [self frameForPageAtIndex:index];
    CGRect playButtonFrame = playButton ? playButton.frame : [self frameForPlayButton:[[UIButton alloc] initWithFrame:CGRectMake(0, 0, 74, 74)] atIndex:index];
    CGFloat width = MIN(pageFrame.size.width - 40, MAX(label.bounds.size.width + 20, 120));
    CGFloat height = MAX(label.bounds.size.height, 20);
    return CGRectIntegral(CGRectMake(floorf(CGRectGetMidX(playButtonFrame) - width / 2),
                                     CGRectGetMaxY(playButtonFrame) + 12,
                                     width,
                                     height));
}
...
- (void)updateNavigation {
    ...
    _qoynruNextButton.enabled = (_currentPageIndex < numberOfPhotos - 1);
        ...
        [photo getVideoURL:^(NSURL *url) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (strongSelf->_currentVideoIndex != index || !strongSelf->_viewIsActive) {
                    [strongSelf setVideoLoadingIndicatorVisible:NO atPageIndex:index];
                    return;
                }
                if (url) {
                    [weakSelf _playVideo:url atPhotoIndex:index];
                } else {
                    [weakSelf showVideoStatusText:@"无法播放" atPageIndex:index autoHide:YES];
                }
            });
        }];
...
- (void)_playVideo:(NSURL *)videoURL atPhotoIndex:(NSUInteger)index {
    ...
    if (!videoURL) {
        [self showVideoStatusText:@"无法播放" atPageIndex:index autoHide:YES];
        return;
    }
    ...
    self.currentVideoPlayerViewController = playerViewController;
    
    __weak typeof(self) weakSelf = self;
    [self presentViewController:playerViewController animated:YES completion:^{
        [weakSelf.currentVideoPlayerViewController.player play];
    }];
}
...
- (void)clearCurrentVideo {
    if (self.currentVideoPlayerViewController) {
        [self.currentVideoPlayerViewController.player pause];
        if (self.currentVideoPlayerViewController.presentingViewController) {
            [self.currentVideoPlayerViewController dismissViewControllerAnimated:NO completion:nil];
        }
...
- (void)setVideoLoadingIndicatorVisible:(BOOL)visible atPageIndex:(NSUInteger)pageIndex {
    MWZoomingScrollView *page = [self pageDisplayedAtIndex:pageIndex];
    page.videoLoadingLabel.text = @"视频加载中...";
    [page.videoLoadingLabel sizeToFit];
    page.videoLoadingLabel.hidden = !visible;
    if (visible && page.videoLoadingLabel) {
        page.videoLoadingLabel.frame = [self frameForVideoLoadingLabel:page.videoLoadingLabel belowPlayButton:page.playButton atIndex:pageIndex];
        [_pagingScrollView bringSubviewToFront:page.videoLoadingLabel];
    }
}

- (void)showVideoStatusText:(NSString *)text atPageIndex:(NSUInteger)pageIndex autoHide:(BOOL)autoHide {
    MWZoomingScrollView *page = [self pageDisplayedAtIndex:pageIndex];
    if (!page.videoLoadingLabel) {
        return;
    }
    
    page.videoLoadingLabel.text = text;
    [page.videoLoadingLabel sizeToFit];
    page.videoLoadingLabel.frame = [self frameForVideoLoadingLabel:page.videoLoadingLabel belowPlayButton:page.playButton atIndex:pageIndex];
    page.videoLoadingLabel.hidden = NO;
    [_pagingScrollView bringSubviewToFront:page.videoLoadingLabel];
    
    if (autoHide) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            MWZoomingScrollView *currentPage = [self pageDisplayedAtIndex:pageIndex];
            currentPage.videoLoadingLabel.hidden = YES;
            currentPage.videoLoadingLabel.text = @"视频加载中...";
        });
    }
}
...
- (void)saveAct {
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
        if (status == PHAuthorizationStatusAuthorized) {
            ...
            if([photo respondsToSelector:@selector(isVideo)] && photo.isVideo) {
                [photo getVideoURL:^(NSURL *videoURL) {
