...
- (NSURL *)localVideoURLWithPath:(NSString *)path {
    if (!path.length) {
        return nil;
    }
...
- (void)getVideoURL:(void (^)(NSURL *url))completion {
    if ([self.message.content isKindOfClass:[WFCCVideoMessageContent class]]) {
        WFCCVideoMessageContent *videoContent = (WFCCVideoMessageContent *)self.message.content;
        NSURL *localURL = [self localVideoURLWithPath:videoContent.localPath];
        if (localURL) {
            completion(localURL);
            return;
        }
        
        if (self.message.messageUid > 0 && videoContent.remoteUrl.length) {
            __weak typeof(self) weakSelf = self;
            BOOL downloading = [[AIOIUEHMediaMessageDownloader sharedDownloader] tryDownload:self.message success:^(long long messageUid, NSString *localPath) {
                NSURL *downloadedURL = [weakSelf localVideoURLWithPath:localPath];
                if (downloadedURL) {
            ...
            localURL = [self localVideoURLWithPath:videoContent.localPath];
            if (localURL) {
                completion(localURL);
                return;
            }
...
- (void)imageLoadingComplete {
    ...
    if(self.underlyingImage && [self.message.content isKindOfClass:[WFCCImageMessageContent class]]) {
        ...
        if(!imgCnt.localPath.length) {
            ...
            NSString *cacheDir = [[AIOIUEHConfigManager globalManager] cachePathOf:self.message.conversation mediaType:Media_Type_IMAGE];
