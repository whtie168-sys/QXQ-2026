...
static AIOIUEHMediaMessageDownloader *sharedSingleton = nil;

@interface AIOIUEHMediaMessageDownloader()
@property(nonatomic, strong)NSMutableDictionary<NSString*, NSNumber*> *downloadingMessages;
@end



@implementation AIOIUEHMediaMessageDownloader
+ (instancetype)sharedDownloader {
    if (sharedSingleton == nil) {
        @synchronized (self) {
            if (sharedSingleton == nil) {
                sharedSingleton = [[AIOIUEHMediaMessageDownloader alloc] init];
...
- (BOOL)tryDownload:(WFCCMessage *)msg
    ...
    if (mediaContent.localPath.length && [AIOIUEHUtilities isFileExist:mediaContent.localPath]) {
    ...
    NSString *cacheDir = [[AIOIUEHConfigManager globalManager] cachePathOf:msg.conversation mediaType:payload.mediaType];
    ...
    savedPath = [AIOIUEHUtilities getUnduplicatedPath:savedPath];
    ...
    if (mediaContent.remoteUrl) {
        [self downloadFileWithURL:mediaContent.remoteUrl parameters:nil savedPath:savedPath downloadSuccess:^(NSURLResponse *response, NSURL *filePath) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [[AIOIUEHMediaMessageDownloader sharedDownloader].downloadingMessages removeObjectForKey:mediaContent.remoteUrl];
...
- (BOOL)tryDownload:(NSString *)mediaPath
    ...
    if(msg) {
        WFCCMediaMessagePayload *payload = (WFCCMediaMessagePayload *)[msg.content encode];
        cacheDir = [[AIOIUEHConfigManager globalManager] cachePathOf:msg.conversation mediaType:payload.mediaType];
    ...
    [self downloadFileWithURL:mediaPath parameters:nil savedPath:savedPath downloadSuccess:^(NSURLResponse *response, NSURL *filePath) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [[AIOIUEHMediaMessageDownloader sharedDownloader].downloadingMessages removeObjectForKey:mediaPath];
